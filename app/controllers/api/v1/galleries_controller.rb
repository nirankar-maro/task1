class Api::V1::GalleriesController < Api::V1::BaseController
  ...
  def index
    ...
    if @gallery.present?
      ...
      render json: {Message: "Successful" ,data: @gal_data_array,status: "Success"}
    else
      render json: {Err_msg: "No Image is available",status: "Failure"}
    end
  end
  ...
  def create
    ...
    if @gallery.save
      ...
      render json: {Err_msg: @gallery.errors.full_messages,status: "Failure"}
    end
  end
  private
